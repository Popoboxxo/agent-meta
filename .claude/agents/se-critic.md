...
## Decision Logic
Up to `max_iterations: 3`. Verdicts:
...
## Correction Loop
- `rejected` (Requirements): `correction_hints` → `se-requirements`. Max `3` iterations.
- `rejected` (Architecture): `correction_hints` → `se-architect`. Max `3` iterations.
