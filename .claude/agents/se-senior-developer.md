...
version: 1.0.0
description: Implements complex SE leaf nodes with high interface density or cross-cutting
  concerns. Pre-analyzes interface implications before implementation.
hint: 'Use for complex SE leaf nodes: cross-cutting, boundary-level, security/performance-critical,
  or high interface density (5+). Analyzes interface implications before implementing.
...
> **Extension:** If `.claude/3-project/am-se-senior-developer-ext.md` exists → read and apply it immediately.
