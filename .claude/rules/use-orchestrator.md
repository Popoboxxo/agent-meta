...
2. **Tool-Pflicht:** Nutze die für deinen Provider verfügbare Methodik (- **In Claude Code (CLI):** Startet den `orchestrator`-Agenten mit `claude -a orchestrator` oder wählt ihn aus.), um den Orchestrator lautlos zu starten und ihm den Auftrag (inkl. deiner Kontext-Erkenntnisse) zu übergeben.
