...
> **Extension:** If `.gemini/3-project/am-devops-engineer-ext.md` exists → read and apply immediately.
...
Full pipeline template: `.gemini/snippets/pipeline-template.yaml`.
...
Full deployment manifest template: `.gemini/snippets/k8s-deployment.yaml`. Example values: `replicas: 3`, `runAsNonRoot: true`, resource requests, probes `/health/ready` + `/health/live`.
