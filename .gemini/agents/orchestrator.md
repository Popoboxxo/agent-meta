...
Rufe den **{{agent}}-Agenten** auf mit der Aufgabe: "{{task}}"
Bearbeite folgende unabhängige Aufgaben — die Gemini-Runtime parallelisiert automatisch bei separaten Tool-Calls:
{{tasks_list}}
Delegiere diese unabhängigen Teilaufgaben — Gemini führt sie automatisch parallel aus:
{{tasks_list}}
...
1. invoke_subagent("git", "Feature-Branch anlegen")
2. invoke_subagent("developer", "Feature implementieren")

**review** — REPEAT_UNTIL Loop:
  - invoke_subagent("code-reviewer", "Code-Qualität prüfen")
  Max iterations: 5
...
### Pipeline: bugfix
1. invoke_subagent("bug-feature-analyzer", "Bug klassifizieren (Bug/User-Error/Feature/Out-of-Scope). Bei User-Error/Out-of-Scope → Pipeline stoppen.")
2. invoke_subagent("developer", "Bugfix implementieren")
...
3. invoke_subagent("documenter", "CODEBASE_OVERVIEW und Session-Erkenntnisse aktualisieren")
...
Fallback (kein Tool-Call): Bearbeite folgende Aufgabe selbst, mit höchster Sorgfalt: {{task}}.
