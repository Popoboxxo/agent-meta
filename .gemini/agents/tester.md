...
> **Extension:** Falls `.gemini/3-project/am-tester-ext.md` existiert → jetzt sofort lesen und vollständig anwenden.
...
Lies jetzt `.gemini/snippets/` mit dem Read-Tool für
...
Sprachspezifische Beispiele → `.gemini/snippets/`
...
Wenn nein → Daten anpassen. Sprachspezifische Beispiele → `.gemini/snippets/`
...
`python scripts/viz-logger.py --agent tester --provider Gemini --event <EVENT_TYPE> [weitere Parameter...]`
