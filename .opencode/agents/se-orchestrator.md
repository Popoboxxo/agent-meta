...
1. **Wait for completeness:** All interface contracts of the leaf nodes MUST be registered in the interface-registry by `se-interface-mgr` BEFORE any leaf is dispatched. No leaf implementation may start while interface contracts are still pending.
2. **Domain gate:** Only leafs with `domain: software` are dispatched to a developer. `domain: hardware` and `domain: mechanics` are documented as COTS specs or stubs (no implementation).
3. **Per-leaf tier selection:** Apply the routing matrix below to each software leaf independently.
4. **Parallel dispatch:** Independent leafs (no shared interface mutations) are dispatched in parallel; respect `max_parallel_cells`.

### Tier Selection Matrix

| Tier | Use when |
|------|----------|
| `se-junior-developer` | Trivial leaf: `domain: software`, 0–1 interfaces in `propagation_map`, no cross-cutting concerns, no boundary-level traits, single-purpose adapter/wrapper. |
| `se-developer` | Standard leaf: `domain: software`, 2–4 interfaces in `propagation_map`, contained within one module, no cross-cutting concerns surfaced. |
| `se-senior-developer` | Complex leaf: `domain: software` AND at least one trait: 5+ interfaces, cross-cutting (transport + persistence + observability), boundary-level (decomposition endpoint), security/performance-critical (auth, crypto, secrets, latency-bound, resource-bound). |

### Routing Inputs (passed to each developer)

For every dispatched leaf, the orchestrator hands over an A2A envelope containing in `payload`:
- `leaf_id` — identifier from termination output
- `req_id` — REQ-ID from REQUIREMENTS.md
- `domain` — `software` (others are not dispatched)
- `description` — black-box description of the leaf
- `interface_specs` — full contract block from `se-interface-mgr` (`inherited_external`, `new_internal_incoming`, `new_internal_outgoing`)
- `propagation_map` — interfaces this leaf inherits/creates
- `acceptance_criteria` — derived from leaf requirements
- `context_boundary` — directory/module scope

### Escalation Handling

Developer escalations (`status: escalate`) are re-routed by the orchestrator:
- `recommended_tier: se-developer` (from junior) → dispatch to `se-developer` with original `findings` in `payload.ctx`
- `recommended_tier: se-senior-developer` (from junior or developer) → dispatch to `se-senior-developer`
- Interface change required → route to `se-interface-mgr` / `se-architect` before any re-dispatch
- Boundary crossing detected → route to `se-architect`

### De-Escalation Hints

When a developer returns `de_escalation_hint: <cheaper-tier>` for a leaf, record it so future dispatches for similar leafs prefer the cheaper tier.
