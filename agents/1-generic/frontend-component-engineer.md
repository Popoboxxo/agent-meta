...
6. **Accessibility baseline (build, not audit):** semantic HTML before an ARIA substitute, keyboard operability (tab order, visible focus, no traps), the correct ARIA pattern for the component type (e.g. combobox pattern) — resolve the pattern from the ARIA Authoring Practices Guide (`reference_standards` "ARIA Authoring Practices Guide (APG)"), not from memory. This installs the baseline — the binding WCAG A/AA/AAA verdict stays with `accessibility-specialist`.
