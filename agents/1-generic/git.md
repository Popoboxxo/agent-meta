...
**Trigger:** after a successful merge — a local merge you performed or a PR observed as merged. Offer to clean up the merged source branch (base = the branch merged into, usually `{{GIT_MAIN_BRANCH}}`).
...
git branch -r --merged {{GIT_MAIN_BRANCH}}
```
