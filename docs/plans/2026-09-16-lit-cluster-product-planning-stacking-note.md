...
| Grund für die Stacking-Phase | `reference_standards` (#801) war die Grammatik, gegen die die Einträge aus #787/#793 validiert werden |
...
- `feat/lit-cluster-review-security` (PR #816, aus #790 + #792) war ein **eigener** gestackter
  ...
  dupliziert, um keinen Konflikt zwischen den gestackten Cluster-PRs zu erzeugen. Der Branch
  ist inzwischen ebenfalls auf `origin/main` rebased (PR-Basis `main`).
- `feat/lit-cluster-core-agents` (PR #817, aus #795 + #791) war ebenfalls ein eigener
  gestackter Branch auf `a59623c2`; inzwischen ebenfalls auf `main` rebased.
