...
> Status: **Konzept v1.0** | 2026-06-28
...
```markdown
## 5. **Subagent-Spawn Singleton-Regel**

`subagent_type="orchestrator"` darf NUR durch `caller=main_chat` aufgerufen werden.

**Provider-Implementierung (durchgesetzt via `role-defaults.yaml:spawn_policy`):**
- **Claude:** `Task` und `Agent` aus `tools:`-Liste der Worker-Agents entfernt ODER Body-Constraint-Liste
- **Opencode:** `permission.task: deny` und `permission.Agent: deny` im Frontmatter (closed mode); `permission.task: allow` + Body-Constraint (pair_whitelist mode)
- **Gemini:** Session-Bootstrap-Constraint-Text
- **Continue/Copilot:** Body-Constraint (kein nativer Mechanismus)

**Verstoß-Verhalten:**
| Verstoß | Aktion |
|---------|--------|
| Worker ruft `subagent_type=orchestrator` | Provider-Block (Opencode) / Tool-Fehler (Claude) / Bootstrap-Reject (Gemini) / unkontrolliert (Continue, Copilot) |
| Self-Spawn (`source==target`) | HARD REJECT (Gate #1) |
```
...
| 5 | `rules/1-generic/a2a-delegation-gates.md` | Gate #5: Singleton-Regel | 15 min |
...
Commits:
  ...
  5. rules(a2a): add Gate #5 — Subagent-Spawn Singleton
...
| "Kein technischer Mechanismus, nur Doku" | **RESOLVED** — Opencode-Permissions + Claude-Frontmatter-Filter |
...
| Implementation auf Branch `feat/orchestrator-singleton-guard` | ⏳ ausstehend |
| Merge in `feat/prompt-modernization-poc` | ⏳ ausstehend |

**Nächster Schritt:** Implementation durch `developer`-Agent auf neuem Branch `feat/orchestrator-singleton-guard` (basierend auf `feat/prompt-modernization-poc`).

---

**Status:** Konzept v1.0 — umsetzungsreif
**Nächster Schritt:** Branch `feat/orchestrator-singleton-guard` anlegen, Implementation starten
