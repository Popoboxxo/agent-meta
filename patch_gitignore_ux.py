...
# Ersetze viewProjectGitignore von 'async function viewProjectGitignore() {' bis '/* ------------------------------- Gitignore Defaults ------------------------------ */'

old_match = re.search(r'async function viewProjectGitignore\(\) \{.*?(?=\/\* ------------------------------- Gitignore Defaults ------------------------------ \*\/)', content, re.DOTALL)
...
new_func = """async function viewProjectGitignore() {
  ...
  const makeToggle = (ignored, onChange) => {
     ...
     const btnIgnore = el("div", { style: `padding: 4px 12px; cursor: pointer; transition: all 0.2s; ${isIgnored ? "background: #2563eb; color: white; font-weight:bold;" : "background: var(--bg-card); color: var(--text-muted);"}` }, ["IGNORED"]);
     const btnTrack = el("div", { style: `padding: 4px 12px; cursor: pointer; transition: all 0.2s; ${!isIgnored ? "background: #d97706; color: white; font-weight:bold;" : "background: var(--bg-card); color: var(--text-muted);"}` }, ["TRACKED"]);
  ...
  const getExceptionsOnSave = []; // We will populate this on Save
  ...
  const renderCategory = (title, desc, catDefault, isChecked, paths) => {
    ...
    const childrenToggles = []; // Store references to children toggles to sync them

    const parentToggle = makeToggle(isChecked, (newVal) => {
        // When parent changes, force all children to match (no exceptions initially)
    ...
    paths.forEach(path => {
      ...
      const childToggle = makeToggle(isPathIgnored, (newVal) => {
          // If child changes independently, it might create an exception. 
          // We don't need to do anything here, state is resolved on Save.
      });
  ...
  saveBtn.onclick = async () => {
    ...
    const ex1 = localSection.computeExceptions();
    const ex2 = genSection.computeExceptions();
    const ex3 = setSection.computeExceptions();
    const finalExceptions = [...ex1, ...ex2, ...ex3];
...
if old_match:
    content = content.replace(old_match.group(0), new_func)
    with open("docs/ui/admin-ui.html", "w", encoding="utf-8") as f:
        f.write(content)
    print("Gitignore UX updated.")
else:
