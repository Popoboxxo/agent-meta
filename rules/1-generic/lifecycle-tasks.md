...
Beim Start einer neuen Konversation: prüfe ob `{{PENDING_TASKS_FILE}}` existiert.

## Pflicht beim Konversations-Start

```bash
# Prüfen ob Lifecycle-Tasks ausstehen
test -f {{PENDING_TASKS_FILE}} && cat {{PENDING_TASKS_FILE}}
```
...
4. Nach Erledigung aller Tasks: lösche `{{PENDING_TASKS_FILE}}`.
...
Datei nicht committen — sie ist gitignored (`{{PENDING_TASKS_FILE}}`).
