...
def _write_release_gate_checksums(
    ...
    if manifest_path.exists():
        for line in manifest_path.read_text(encoding="utf-8").splitlines():
            # This exact static header block is always re-emitted fresh below
            # -- preserving a prior run's copy of it here duplicated it once
            # per sync.py run with no bound (a comment line has no
            # `_checksum_entry_name`, so it always fell into the "preserve
            # verbatim" branch below, header included).
