...
def _generate_rule_content(server_name: str, server_def: dict, compact: bool = False) -> str:
    ...
    if conn:
        ...
        if compact:
            ...
            lines.append(f"**Verbindungstyp:** `{conn_type}` — Details: `config/plugin-catalog.yaml`.")
            lines.append("")
        else:
...
def sync_secrets_template(
    ...
    from config/plugin-catalog.yaml.
...
def generate_mcp_artifacts(
    ...
    if pc.get("has_rules"):
        ...
        previously_managed = bootstrap_previously_managed(
            ...
            content_marker="config/plugin-catalog.yaml` — nicht manuell bearbeiten",
        )
