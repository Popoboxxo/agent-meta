...
# ---------------------------------------------------------------------------
# Meta-repo documentation scaffolding
# ---------------------------------------------------------------------------

def scaffold_meta_repo_docs(
    agent_meta_root: Path,
    project_root: Path,
    log,
    dry_run: bool = False,
) -> list[str]:
    """Auto-generate meta-repo knowledge files when meta-repo: true.

    Creates docs/PATTERNS.md, docs/LEARNINGS.md, docs/CONVENTIONS.md.
    Uses starter templates from .agent-meta/templates/ if they exist,
    otherwise creates files with a standard header.
    ...
    # Starter template locations in agent-meta
    template_files = {
    ...
    for filename, template_rel in template_files.items():
        dest = docs_dir / filename
        if dest.exists():
            log.info("meta-repo-docs", f"already exists — skipping: {filename}")
            continue

        # Check for starter template in agent-meta
        starter = agent_meta_root / "templates" / template_rel
        if starter.exists():
            content = starter.read_text(encoding="utf-8")
        else:
            content = _default_meta_repo_doc(filename)

        if dry_run:
            created.append(str(dest.relative_to(project_root)))
            log.info("meta-repo-docs", f"would create: {filename}")
            continue

        dest.parent.mkdir(parents=True, exist_ok=True)
        dest.write_text(content, encoding="utf-8")
        created.append(str(dest.relative_to(project_root)))
        log.info("meta-repo-docs", f"created: {filename}")

    return created


def _default_meta_repo_doc(filename: str) -> str:
    """Return a default header for a meta-repo knowledge file."""
    headers = {
        "PATTERNS.md": (
            "# Cross-Plugin Patterns\n\n"
            "> Auto-generated by agent-meta sync.py (meta-repo mode)\n\n"
            "Document reusable patterns that apply across all plugins in this workspace.\n\n"
        ...
        "LEARNINGS.md": (
            "# Lessons Learned\n\n"
            "> Auto-generated by agent-meta sync.py (meta-repo mode)\n\n"
            "Capture insights, gotchas, and solutions discovered during development.\n\n"
            "## Recent Learnings\n\n"
            "## Archived Learnings\n\n"
        ),
        "CONVENTIONS.md": (
            ...
            "> Auto-generated by agent-meta sync.py (meta-repo mode)\n\n"
            "Define standards that all plugins in this workspace must follow.\n\n"
            "## Naming Conventions\n\n"
            "## Code Style\n\n"
            "## Git Workflow\n\n"
