#!/bin/bash
# Scenario assert for 03-plugin-catalog
#
...
if [ -z "$REPO_ROOT" ]; then
    echo "ASSERT ERROR (03-plugin-catalog): REPO_ROOT missing (pass as \$1 or env var)"
    exit 2
fi

fail() {
    echo "ASSERT FAIL (03-plugin-catalog): $*"
    exit 1
}
...
echo "ASSERT OK (03-plugin-catalog): all marker files present"
