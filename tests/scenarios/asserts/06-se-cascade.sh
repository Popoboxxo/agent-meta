#!/bin/bash
# Scenario assert for 06-se-cascade
#
...
if [ -z "$REPO_ROOT" ]; then
    echo "ASSERT ERROR (06-se-cascade): REPO_ROOT missing (pass as \$1 or env var)"
    exit 2
fi

fail() {
    echo "ASSERT FAIL (06-se-cascade): $*"
    exit 1
}
...
echo "ASSERT OK (06-se-cascade): all marker files present"
