#!/bin/bash
# Scenario assert for 08-release-automation
#
...
if [ -z "$REPO_ROOT" ]; then
    echo "ASSERT ERROR (08-release-automation): REPO_ROOT missing (pass as \$1 or env var)"
    exit 2
fi

fail() {
    echo "ASSERT FAIL (08-release-automation): $*"
    exit 1
}
...
echo "ASSERT OK (08-release-automation): all marker files present"
