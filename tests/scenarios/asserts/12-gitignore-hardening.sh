#!/bin/bash
# Scenario assert for 12-gitignore-hardening
#
...
if [ -z "$REPO_ROOT" ]; then
    echo "ASSERT ERROR (12-gitignore-hardening): REPO_ROOT missing (pass as \$1 or env var)"
    exit 2
fi

fail() {
    echo "ASSERT FAIL (12-gitignore-hardening): $*"
    exit 1
}
...
echo "ASSERT OK (12-gitignore-hardening): all marker files present"
