#!/bin/bash
# Scenario assert for 17-orchestrator-main-chat
#
...
if [ -z "$REPO_ROOT" ]; then
    echo "ASSERT ERROR (17-orchestrator-main-chat): REPO_ROOT missing (pass as \$1 or env var)"
    exit 2
fi

fail() {
    echo "ASSERT FAIL (17-orchestrator-main-chat): $*"
    exit 1
}
...
echo "ASSERT OK (17-orchestrator-main-chat): all marker files present"
