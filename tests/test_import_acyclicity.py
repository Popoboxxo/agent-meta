...
Builds the module graph from TOP-LEVEL relative imports only (stdlib ``ast``,
Tarjan SCC) and asserts the graph is a DAG: no strongly connected component
with more than one module. Function-local (lazy) imports are deliberately
...
def _top_level_relative_imports(tree: ast.Module) -> list[tuple[int, str, str | None]]:
    """Top-level relative imports as (level, module, alias_name) triples.

    ``from .x import y``      → (1, "x", "y")
    ``from . import x``       → (1, None, "x")
    ``from ..x.y import z``   → (2, "x.y", "z")

    ``if TYPE_CHECKING:`` blocks are skipped — type-only back-edges are not
    import-time edges.
    """
...
def _build_graph() -> dict[str, set[str]]:
    ...
    for mod_name, path in _iter_lib_modules():
        ...
        deps: set[str] = set()
        for level, module, alias in _top_level_relative_imports(tree):
...
def test_top_level_import_graph_is_acyclic():
    ...
    assert not cycles, (
        "Import-time cycles detected in scripts/lib (top-level relative imports): "
        f"{cycles}. Break the cycle with a leaf/orchestration module "
...
def test_known_edges_are_resolved():
    """Sanity: relative imports must resolve to real lib modules (a silent
    resolution failure would make the acyclicity check vacuous)."""
    graph = _build_graph()
